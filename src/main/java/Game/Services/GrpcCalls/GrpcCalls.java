    ...
    public static void coordinatorCallAsync(String serverAddress) throws InterruptedException {
        ...
        stub.election(request, new StreamObserver<PlayerMessageResponse>() {
            ...
            public void onNext(PlayerMessageResponse response) {
                System.out.println("GRPCalls, coordinatorCallAsync: GOT COORDINATOR MESSAGE " + response);
            }
            ...
            public void onNext(PlayerMessageResponse response) {
                ...
                if (MessageType.valueOf(response.getMessageType()) == MessageType.GREETING_OK) {
                    ...
                    if (requestGameState.equals(GameState.ELECTION_MESSAGES_SENT)) {
                        ...
                        double otherPlayerDistance = calculateDistanceToNearestBasePoint(Double.parseDouble(response.getPosX()), Double.parseDouble(response.getPosY()));
                        System.out.println("GRPCalls, greetingCallAsync: Player " + myId + ": GREETING_OK election message, if I don't get OK messages in 3s I will change to leader");
                        PlayerExtended myPlayer = GlobalState.getStateObject().getMyPlayer();
                        ...
                        if (greetingElectionFutureProcessed.putIfAbsent("ELECTION", true) == null) {
                            System.out.println("GRPCalls, greetingCallAsync: Player " + myId + ": ELECTION !FROM GREETING! message put to map, and I will become leader in 3 s");
                            ...
                            timeoutFutureHolderGreetingElection[0] = executor.schedule(() -> {
                                ...
                                System.out.println("GRPCalls, greetingCallAsync: Player " + myId + ": timeout occurred. State set to LEADER and sending COORDINATOR message ");
                            }, 12, TimeUnit.SECONDS);
                        }
                    }
                }
